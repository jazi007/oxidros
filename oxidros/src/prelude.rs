...
// Note: We only re-export `Error`, not `Result` to avoid shadowing `std::result::Result`.
// Users should use `oxidros_core::Result` explicitly if needed.
...
// Re-export message crate for generated types
pub use oxidros_msg;
