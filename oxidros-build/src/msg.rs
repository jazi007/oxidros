...
struct Ros2Callbacks;

impl ParseCallbacks for Ros2Callbacks {
    ...
    fn add_attributes(&self, info: &ItemInfo) -> Vec<String> {
        ...
        vec![
            ...
            // When not using rcl, derive Serialize/Deserialize for CDR encoding.
            // Use serde(crate = "...") to tell serde's derive macros where to find serde,
            // since it's re-exported through ros2_types rather than a direct dependency.
    ...
    fn add_field_attributes(&self, field_info: &FieldInfo) -> Vec<String> {
        // Check if this is a large array (> 32 elements)
...
pub fn generate_msgs(packages: &[&str]) {
    ...
    Generator::new()
        ...
        .derive_debug(true)
        .parse_callbacks(Box::new(Ros2Callbacks))
        .includes(all_files)
