// Auto-generated ROS2 message types
// Generated by ros2msg from all ROS2 packages

#[repr(C)]
#[derive(Debug, ros2_types::Ros2Msg, ros2_types::TypeDescription)]
#[ros2(package = "rcl_interfaces", interface_type = "msg")]
#[cfg_attr(
    not(feature = "rcl"),
    derive(ros2_types::serde::Serialize, ros2_types::serde::Deserialize)
)]
#[cfg_attr(not(feature = "rcl"), serde(crate = "ros2_types::serde"))]
pub struct ParameterEventDescriptors {
    pub new_parameters: super::super::super::rcl_interfaces::msg::parameter_descriptor::ParameterDescriptorSeq<
        0,
    >,
    pub changed_parameters: super::super::super::rcl_interfaces::msg::parameter_descriptor::ParameterDescriptorSeq<
        0,
    >,
    pub deleted_parameters: super::super::super::rcl_interfaces::msg::parameter_descriptor::ParameterDescriptorSeq<
        0,
    >,
}
